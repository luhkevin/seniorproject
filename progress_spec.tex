...
\label{sec:intro}
Autoassociative memories are a model that are frequently used in machine learning applications. In 
order to faciliate dealing with these autoassociative memories, this project attempts to ameliorate 
dealing with structured data in the machine learning pipeline. Machine learning deals 
with extracting features from these structured data, but these data can become quite
complicated and cumbersome to process. While strings or images may just store data as 
word count or pixel value, other data such lists or trees can become more complicated due to their 
recursive nature. This programming library interface will handle extracting features from structured 
...
network. There is no conventional way to do this conversion as it is a relatively new field, but
Sperduti's~\cite{sperduti} approach is one possible solution.
