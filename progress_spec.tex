...
%TODO cite the deeplearning website
...
than converting it to feature vectors. In relation to this, Alessandro Sperduti %cite
...
While kernel methods exist for a vast array of problems and data sets, Collins and Duffy %cite
...
In terms of the query interface, it currently contains all the functionality needed for basic queries.
This includes the basic ``wildcard" and ``constant" queries, as well as support for ``pair" and ``either"
queries. This was created using the standard OCaml interface setup. In the future, extra queries
will be needed to deal with structured data, but the interface will be easily extensible due to
the flexible type system of OCaml. The interface also has the ability to use different types
of underlying mechanisms as a memory (Figure \ref{fig:queryint}).
...
We have created one possible type of memory using the autoencoders discussed in the related work
section. An autoencoder uses hidden layers (matrices) to perform transformations on input vectors,
compressing them to a lower dimension and doing a reverse transformation to retrieve them. 
Let the input vector be $x$. We can transform $x$ to a lower dimension vector $y$ by applying a
transformation matrix $W$ and adding a bias value $b$, then applying a non-linear transformation
$s$, such as the sigmoid function.
...
a different matrix $W'$ and bias factor $b'$.

\begin{gather}
z = s(W' y  + b')
\end{gather}

It learns the correct matrices by comparing the input value $x$
to the retrieved value $z$ using the cross-entropy error rate $L(x,z)$, and adjusting appropriately.
...
to be able to reconstruct partially missing input, as is the goal of our query interface.
