...
\begin{abstract}
    ...
    fidelity. On top of this, our library will provide an intuitive interface to this probabilistic
...
associative memories where we can retrieve memories based similarity between stimuli and the memories themselves. 
Indeed, Norman and Bobrow~\cite{bobrow} construct a memory model which describes how we retrieve 
...
as well as the different types of queries we can make to the memory. The complexity allowed
...
The foremost technical challenges in our project deal with 
...
probabilistic library that demonstrate the its benefits. To accomplish 
...
that our probability model works correctly; that is, that it generates the expected distribution values
based on the given inputs.

In addition, we will demonstrate the logical fluidity of using our probabilistic library:
