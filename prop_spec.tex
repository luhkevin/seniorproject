...
for programming with an underlying memory.
...
and compress the stored values. We will work on different compression methods,
including neural networks and network flow equilibrium. Finally we will create
a type of memory to work with structured data.
...
a lower memory footprint. Regarding other metrics, we will show how using our 
probabilistic library can yield fewer lines of code and has more logical fluidity 
for programs that require statistical and probabilistic methods and 
than a vanilla implementation of the same program in OCaml. 
